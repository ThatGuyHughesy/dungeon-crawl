...
class CampaignInvitesController < ApplicationController
  ...
  def create
    ...
    @campaign_invite.recipient_id = @recipient.id unless @recipient.nil?
    if @campaign_invite.save
      # TODO: remove the DB updates here: we should direct existing users to a campaigns homepage. Once they click the link and go to that page, the logic to add them to the campaign will occur
      if !@recipient.nil?
